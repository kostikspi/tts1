...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        durations = self.get_durations(ind)
        # phonemes = np.array(text_to_sequence(data_dict["text"], ['english_cleaners']))
        target_text = self.get_text(ind)
        phonemes = np.array(text_to_sequence(target_text.strip(), ['english_cleaners']))
        pitch = self.get_pitch(audio_wave)
        energy = self.get_energy(audio_spec)
        out = {
            "audio": audio_wave,
            "mel_target": audio_spec,
            "duration": durations,
            "pitch_target": pitch,
    ...
    @staticmethod
    def get_energy(audio_spec):
        return torch.norm(audio_spec, p=2, dim=1)

    @staticmethod
    def get_pitch(audio_wave):
        pitch = torchaudio.functional.detect_pitch_frequency(audio_wave, 16000)
        return pitch
